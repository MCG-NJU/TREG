...
class FcotTracker(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        self.online_train_x_36_reg = None
        self.max_score_reg = -100
        self.s = None
        self.max_score = -100
    ...
    def track(self, image) -> dict:
        ...
        if getattr(self.params, 'update_classifier_and_regressor', False) and update_flag:
            ...
            online_train_x_72_reg = test_x_72_pyramid
            online_train_x_36_reg = test_x_36_pyramid

            # Create target_box and label for spatial sample
            target_box = self.get_training_box(self.pos, self.target_sz, sample_pos[scale_ind,:], sample_scales[scale_ind])

            if (self.frame_num - 1) % self.params.reg_train_skipping == 0 or (
                    self.frame_num - 1) <= self.params.init_train_frames:
                self.update_regressor(online_train_x_72_reg, online_train_x_36_reg, target_box)
            ...
            if hard_negative:
                ...
                if (self.frame_num - 1) % self.params.train_skipping == 0:
                    if self.params.ues_select_sample_strategy:
                        ...
                        # self.update_regressor(self.online_train_x_72_reg, self.online_train_x_36_reg, target_box)
                    else:
                        self.update_classifier_72(train_x_72_cls, target_box, None, s[scale_ind, ...])
                        self.update_classifier_18(train_x_18_cls, target_box, None, s[scale_ind, ...])
                        # self.update_regressor(online_train_x_72_reg, online_train_x_36_reg, target_box)
                ...
                # if max_score >= self.max_score_reg or self.online_train_x_36_reg is None:
                #     self.max_score_reg = max_score
                #     self.target_box_reg = target_box
                #     self.online_train_x_36_reg = online_train_x_36_reg
                #     self.online_train_x_72_reg = online_train_x_72_reg
                # if (self.frame_num - 1) % self.params.reg_train_skipping == 0:
                #     self.update_regressor(online_train_x_72_reg, online_train_x_36_reg, target_box)
                    # self.update_regressor(self.online_train_x_72_reg, self.online_train_x_36_reg, self.target_box_reg)
                    # self.online_train_x_36_reg = None
                    # self.max_score_reg = -100
    ...
    def init_classifier_and_regressor(self, init_backbone_feat):
        ...
        self.init_reg_target_boxes = target_boxes.clone()[:self.params.reg_init_memory]
        self.init_reg_train_feat = train_feat_72_reg.clone()[:self.params.reg_init_memory]
        self.init_reg_targets = None
        self.init_reg_mask_label = None
        ...
        with torch.no_grad():
            ...
            if reg_num_iter > 0:
                # print(reg_num_iter)
                self.target_reg_filter_72, _, _, self.init_reg_targets, self.init_reg_mask_label = self.net.regressor_72.filter_optimizer(
                    init_reg_filter,
                    train_feat_72_reg[:self.params.reg_init_memory],
                    target_boxes.view(-1, 4).clone()[:self.params.reg_init_memory],
            ...
            self.init_target_filter_72 = self.target_filter_72.clone()
            self.init_target_filter_18 = self.target_filter_18.clone()
            # self.init_reg_filter = init_reg_filter
            self.init_reg_filter = self.target_reg_filter_72.clone()
    ...
    def update_regressor_init(self, train_x_reg_72, target_box, learning_rate=None, scores=None):
        # Set flags and learning rate
        hard_negative_flag = learning_rate is not None
        if learning_rate is None:
            learning_rate = self.params.learning_rate
    ...
    def update_regressor(self, train_x_72, train_x_36, target_box):
        # Set flags and learning rate
        # get init_reg_filter using target sample and optimize filters using training samples
        # with torch.no_grad():
        #     cur_reg_filter = self.net.regressor_72.generate_init_filter(train_x_36,
        #                                                                 train_x_72,
        #                                                                 target_box)
        #     print(cur_reg_filter.shape)
        with torch.no_grad():
            target_feat_36 = train_x_36.view(-1, *train_x_36.shape[-3:])[0].unsqueeze(0).to(self.target_reg_filter_72.device)
            target_feat_72 = train_x_72.view(-1, *train_x_72.shape[-3:])[0].unsqueeze(0).to(self.target_reg_filter_72.device)
            target_bb = target_box.unsqueeze(0).clone().to(self.target_reg_filter_72.device)
            cur_reg_filter = self.net.regressor_72.generate_init_filter(target_feat_36, target_feat_72, target_bb)

        # Decide the number of iterations to run
        reg_num_iter = 0
        if (self.frame_num - 1) <= self.params.init_train_frames:
            reg_num_iter = self.params.reg_init_train_iter
        elif (self.frame_num - 1) % self.params.train_skipping == 0:
            reg_num_iter = getattr(self.params, 'reg_net_opt_update_iter', None)
        ...
        if reg_num_iter > 0:
            # Run the model optimizer module
            with torch.no_grad():
                self.target_reg_filter_72, _, _ = self.net.regressor_72.filter_optimizer(weights=cur_reg_filter,
                                                                                         feat=self.init_reg_train_feat,
                                                                                         bb=self.init_reg_target_boxes,
                                                                                         num_iter=reg_num_iter,
                                                                                         reg_targets=self.init_reg_targets,
                                                                                         mask_label=self.init_reg_mask_label
                                                                                         )