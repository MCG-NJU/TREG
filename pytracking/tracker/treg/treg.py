...
class TREG(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        self.net = copy.deepcopy(self.params.net)
        ...
        # im_patches, init_backbone_feat = self.generate_init_samples_v2(im)

        # self.init_net(im_patches)
    ...
    def track(self, image) -> dict:
        ...
        with torch.no_grad():
            ...
            test_x_72_reg = self.net.regressor_72.extract_regression_feat(test_x_72_pyramid)

            target_boxes_reg = self.target_boxes_reg_72[:self.num_stored_samples_reg_72[0], :].clone()
            samples_72_reg = self.training_samples_reg_72[0][:self.num_stored_samples_reg_72[0], ...]

            w2h2_72, reg_feat_nonlocal = self.net.regressor_72.regress(samples_72_reg, target_boxes_reg, test_x_72_reg)

        feat_nonlocal = torch.mean(test_x_72_reg, dim=1)
        # feat_nonlocal = test_x_18_cls[:, 60, :, :]
        img_patch = self.im_patches[0, ...].clone().squeeze().cpu().permute(1, 2, 0).numpy()
        self.tensors_gray2rgb(feat_nonlocal, img_patch)
        # print("feat_nonlocal shape: {}".format(feat_nonlocal.shape))
        ...
        scores_72_1 = F.interpolate(scores_raw_18, size=(self.feat_sz, self.feat_sz))
        scores_raw_72 = self.classify_target_72(test_x_72_cls)
        scores_72_2 = F.interpolate(scores_raw_72, size=(self.feat_sz, self.feat_sz))
        scores_72 = scores_72_1 * self.params.merge_rate_18 + scores_72_2 * (1 - self.params.merge_rate_18)
        ...
        translation_vec, scale_ind, s, flag, max_disp, max_disp2, max_score = \
                                    self.localize_target(scores_72, sample_scales, feature_sz=self.feat_sz)
        ...
        #
        # save_path = "/home/cyt/data/experiments/lasot-results-super-fcotv3/"
        # if self.frame_num == 2:
    ...
    #     fm = feat.cpu().numpy()  #[256, 72, 72]
    #     fm = np.absolute(fm)
    #     fm = fm.sum(axis=0)
    #     fm = fm[1:-1, 1:-1]
    #     f_max = np.max(fm)
    #     f_min = np.min(fm)
    #     fm = (fm - f_min) / (f_max - f_min)
    #     fm = cv2.resize(fm, (224,224), interpolation=cv2.INTER_NEAREST)
    #
    #     # plt.imsave(save_dir, fm, cmap='viridis')

    def tensors_gray2rgb(self, tensor, image):
        """Convert to rgb images [tenosrs： (num, 1, h, w)].
           :return tensors： (num, 3, h, w)
        """
        image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)

        save_path = '/home/cyt/data/experiments/treg_feat_vis/after_treg_reg_feat/'
        if self.frame_num == 2:
            for i in range(1, 1000):
                    self.seq_path = save_path + str(i)
                    if not os.path.exists(self.seq_path):
                        os.makedirs(self.seq_path)
                        break
        # if not os.path.exists('/home/cyt/data/experiments/treg_feat_vis/before_treg_reg_feat/{}'.format(self.seq_id)):
        #     os.makedirs('/home/cyt/data/experiments/treg_feat_vis/before_treg_reg_feat/{}'.format(self.seq_id))
        # tmp = os.path.join('/home/cyt/data/experiments/treg_feat_vis/before_treg_reg_feat/{}/{}.png'.format(self.seq_id,
        #                                                                                                     self.frame_num))
        # if not os.path.exists('/home/cyt/data/experiments/treg_feat_vis/after_treg_reg_feat/{}'.format(self.seq_id)):
        #     os.makedirs('/home/cyt/data/experiments/treg_feat_vis/after_treg_reg_feat/{}'.format(self.seq_id))
        tmp = os.path.join('{}/{}.png'.format(self.seq_path,self.frame_num))

        hm = tensor.clone().squeeze().cpu().detach().numpy()  # [h, w]

        #
        # min_ = np.min(hm[20:50, 20:50])
        # hm = np.maximum(hm, 0)
        # hm = np.minimum(hm, 0)
        # hm = hm + abs(min_)
        hm = np.fabs(hm)
        # hm = np.absolute(hm)
        max_ = np.max(hm[20:50, 20:50])
        # print("max_v : {}".format(max_))
        hm = np.minimum(hm, max_)
        # hm /= max_
        f_max = np.max(hm)
        f_min = np.min(hm)
        hm = (hm - f_min) / (f_max - f_min)
    ...
    def ctdet_decode(self, w2h2, max_disp):
        ...
        # w2h2_max = w2h2[..., max_disp[1], max_disp[0]]
        return w2h2_max
    ...
    def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        im_patches, patch_coords = sample_patch_multiscale(im, pos, scales, sz,
                                                           mode=getattr(self.params, 'border_mode', 'replicate'),
                                                           max_scale_change=getattr(self.params, 'patch_max_scale_change',
                                                                                            None))
        with torch.no_grad():
    ...
    def generate_init_samples_v2(self, im: torch.Tensor):
        """Perform data augmentation to generate initial training samples."""
        ...
        return im_patches, init_backbone_feat
    ...
    def init_target_boxes(self):
        ...
        self.target_boxes_reg_72 = self.target_boxes[:int(self.params.reg_mem_size)].clone()
        # print("target_boxes shape: {}".format(self.target_boxes_reg_72.shape))
    ...
    def init_memory(self, train_x_72_cls: TensorList,
        ...
        self.num_stored_samples_reg_72 = [int(self.params.reg_mem_size)]
        ...
        self.previous_replace_ind_reg_72 = int(self.params.reg_mem_size-1)
        ...
        self.training_samples_reg_72 = TensorList(
            [x.new_zeros(int(self.params.reg_mem_size), x.shape[1], x.shape[2], x.shape[3]) for x in train_x_72_reg])
    ...
    def update_memory_reg_72(self, sample_x_reg_72: TensorList, target_box, learning_rate=None):
        # Update weights and get replace ind
        replace_ind = [self.previous_replace_ind_reg_72]
        self.previous_replace_ind_reg_72 += 1
        if self.previous_replace_ind_reg_72 == int(self.params.reg_mem_size):
            self.previous_replace_ind_reg_72 = int(self.params.reg_mem_size/2)
        print("replace: {}".format(replace_ind))
        # print(self.sample_weights)
        ...
        if self.num_stored_samples_reg_72[0] < int(self.params.reg_mem_size):
            self.num_stored_samples_reg_72[0] += 1
    ...
    # def init_vsd_model(self, input):
    #     self.classifier_18_ext = copy.deepcopy(self.net.classifier_18)
    #     self.classifier_72_ext = copy.deepcopy(self.net.classifier_72)
    #     self.regressor_72_ext = copy.deepcopy(self.net.regressor_72)
    #
    #     init_backbone_feat = self.net.extract_backbone(input)
    ...
    #
    #     train_feat_18_cls = self.classifier_18_ext.extract_classification_feat(init_backbone_feat['layer3'])
    #     train_feat_72_cls = self.classifier_72_ext.extract_classification_feat(train_feat_72.
    #                                                                            view(-1, *train_feat_72.shape[
    #                                                                                      -3:]))  # [23, 256, 72, 72]
    #     train_feat_72_reg = self.regressor_72_ext.extract_regression_feat(train_feat_72.
    #                                                                       view(-1, *train_feat_72.shape[-3:]))
    #     # print("train_feat_18_cls shape: {}".format(train_feat_18_cls.shape))
    #     # print("train_feat_72_cls shape: {}".format(train_feat_72_cls.shape))
    #     # print("train_feat_72_reg shape: {}".format(train_feat_72_reg.shape))
    #     offset_maps = self.regressor_72_ext.regress_v2(train_feat_72_reg)
    #     heatmap_18 = self.classifier_18_ext.classify_v2(train_feat_18_cls)
    #     heatmap_72 = self.classifier_72_ext.classify_v2(train_feat_72_cls)
    #
    #     return heatmap_18, heatmap_72, offset_maps
    #
    # def _generate_cls_label_18_function(self, target_bb):
    ...
    #     """
    #
    #     gauss_label = prutils.gaussian_label_function(target_bb.view(-1, 4), 1/20, 3, 18, 18*16, end_pad_if_even=True)
    #
    #     return gauss_label
    #
    # def _generate_cls_label_72_function(self, target_bb):
    #     """ Generates the gaussian label function centered at target_bb
    #     args:
    #         target_bb - target bounding box (num_images, 4)
    #
    ...
    #     """
    #
    #     gauss_label = prutils.gaussian_label_function(target_bb.view(-1, 4), 1/40, 3, 72, 18*16, end_pad_if_even=True)
    #
    #     return gauss_label
    #
    # def init_net(self, im_patches: torch.Tensor):
    #     self.net.train()
    #     objective = {'test_clf_72': ltr_losses.LBHinge(threshold=0.05),
    #                  'test_clf_18': ltr_losses.LBHinge(threshold=0.05),
    #                  'reg_72': REGLoss(dim=4)
    #                  }
    #
    #     target_boxes = self.init_target_boxes()[:5].clone().cpu()
    #
    #     ### generate labels
    #     cls_label_18 = self._generate_cls_label_18_function(target_boxes)
    #     cls_label_72 = self._generate_cls_label_72_function(target_boxes)
    #     cls_label_18 = cls_label_18.cuda()
    #     cls_label_72 = cls_label_72.cuda()
    #     # print("init cls label")
    #
    #     boxes_72 = (target_boxes * 0.25).clone().float()
    #     # boxes is in format xywh, convert it to x0y0x1y1 format
    #     boxes_72[:, 2:4] = boxes_72[:, 0:2] + boxes_72[:, 2:4]
    #
    #     center_float = (boxes_72[:, 0:2] + boxes_72[:, 2:4]) * 0.5
    #     center_int = center_float.int().float()
    #     # print("center_float: {}".format(center_float))
    #
    #     ind_72 = center_int[:, 1] * 72 + center_int[:, 0]  # [num_images, 1]
    #     ind_72 = ind_72.long().cuda()
    #
    #     wl_72, wr_72 = center_float[:, 0] - boxes_72[:, 0], boxes_72[:, 2] - center_float[:, 0]
    #     ht_72, hb_72 = center_float[:, 1] - boxes_72[:, 1], boxes_72[:, 3] - center_float[:, 1]
    #     w2h2_72 = torch.stack((wl_72, wr_72, ht_72, hb_72), dim=1).cuda()   # [num_images, 4]
    #     # print("w2h2_72: {}".format(w2h2_72))
    #
    #     print("Online training start...")
    #     t1 = time.time()
    #     for iter in range(2):
    #         inputs = im_patches[:5, ...].clone()
    #         hm_18, hm_72, offset_maps = self.init_vsd_model(inputs)
    #         # print("hm_18 shape: {}, hm_72 shape: {}, off_map shape: {}".format(hm_18.shape, hm_72.shape, offset_maps.shape))
    #
    #         # calulate losses
    #         reg_loss_72 = objective['reg_72'](offset_maps, ind_72, w2h2_72, radius=1)
    #         clf_losses_test_72 = objective['test_clf_72'](hm_72, cls_label_72)
    #         clf_losses_test_18 = objective['test_clf_18'](hm_18, cls_label_18)
    #         tot_loss = reg_loss_72 * 0.1 + clf_losses_test_72 * 100 + clf_losses_test_18 * 100
    #         print("Training {}-iters: Total loss: {}, Clf_18 loss: {}, Clf_72 loss: {}, Reg loss: {}".format(
    #             iter, tot_loss.item(), clf_losses_test_18.item() * 100, clf_losses_test_72.item() * 100,
    #             reg_loss_72.item() * 0.1 ))
    #
    #         optimizer = optim.SGD([{'params': self.net.feature_extractor.parameters()},
    #                                {'params': self.net.pyramid_36.parameters()},
    #                                {'params': self.net.pyramid_72.parameters()},
    #                                {'params': self.regressor_72_ext.parameters()},
    #                                {'params': self.classifier_72_ext.parameters()},
    #                                {'params': self.classifier_18_ext.parameters()},
    #                                {'params': self.net.pyramid_first_conv.parameters()}],
    #                               lr=0.001)
    #         optimizer.zero_grad()
    #         tot_loss.backward()
    #         optimizer.step()
    #     print("online training cost {}s".format(time.time() - t1))
    #     self.net.eval()


    def init_vsd_model(self, input, train_inputs, train_bb):
        # self.classifier_18_ext = copy.deepcopy(self.net.classifier_18)
        # self.classifier_72_ext = copy.deepcopy(self.net.classifier_72)
        # self.regressor_72_ext = copy.deepcopy(self.net.regressor_72)
        init_backbone_feat = self.net.extract_backbone(input)
        x = self.net.get_backbone_clf_feat(init_backbone_feat)
        test_feat_18 = self.net.pyramid_first_conv(x=None, x_backbone=x)
        test_feat_36 = self.net.pyramid_36(test_feat_18, init_backbone_feat['layer2'])
        test_feat_72 = self.net.pyramid_72(test_feat_36, init_backbone_feat['layer1'])
        test_feat_18_cls = self.net.classifier_18.extract_classification_feat(init_backbone_feat['layer3'])
        test_feat_72_cls = self.net.classifier_72.extract_classification_feat(test_feat_72)  # [23, 256, 72, 72]
        test_feat_72_reg = self.net.regressor_72.extract_regression_feat(test_feat_72)

        train_init_backbone_feat = self.net.extract_backbone(train_inputs)
        x = self.net.get_backbone_clf_feat(train_init_backbone_feat)
        train_feat_18 = self.net.pyramid_first_conv(x=None, x_backbone=x)
        train_feat_36 = self.net.pyramid_36(train_feat_18, train_init_backbone_feat['layer2'])
        train_feat_72 = self.net.pyramid_72(train_feat_36, train_init_backbone_feat['layer1'])

        train_feat_18_cls = self.net.classifier_18.extract_classification_feat(train_init_backbone_feat['layer3'])
        train_feat_72_cls = self.net.classifier_72.extract_classification_feat(train_feat_72)  # [23, 256, 72, 72]
        train_feat_72_reg = self.net.regressor_72.extract_regression_feat(train_feat_72)
        # print("train_feat_18_cls shape: {}".format(train_feat_18_cls.shape))
        # print("train_feat_72_cls shape: {}".format(train_feat_72_cls.shape))
        # print("train_feat_72_reg shape: {}".format(train_feat_72_reg.shape))
        # offset_maps = self.regressor_72.regress_v2(train_feat_72_reg)
        # heatmap_18 = self.classifier_18.classify_v2(train_feat_18_cls)
        # heatmap_72 = self.classifier_72.classify_v2(train_feat_72_cls)

        filter_18, filter_iter_18, _ = self.net.classifier_18.get_filter(train_feat_18_cls, train_bb, num_iter=4)
        filter_72, filter_iter_72, _ = self.net.classifier_72.get_filter(train_feat_72_cls, train_bb, num_iter=4)

        heatmap_18 = [self.net.classifier_18.classify(f, test_feat_18_cls) for f in filter_iter_18]
        heatmap_72 = [self.net.classifier_72.classify(f, test_feat_72_cls) for f in filter_iter_72]
        offset_maps = self.net.regressor_72.regress(train_feat_72_reg, train_bb, test_feat_72_reg)

        # print("off shape: {}".format(offset_maps.shape))
    ...
    def _generate_cls_label_18_function(self, target_bb):
        ...
        gauss_label = prutils.gaussian_label_function(target_bb.view(-1, 4), 1/20, 4, 18, 18*16, end_pad_if_even=True)
    ...
    def _generate_cls_label_72_function(self, target_bb):
        ...
        gauss_label = prutils.gaussian_label_function(target_bb.view(-1, 4), 1/40, 4, 72, 18*16, end_pad_if_even=True)

        return gauss_label

    def vis_hm(self, hm, num):
        tmp = os.path.join('/home/cyt/data/experiments/hm_vis/{}.png'.format(num))
        hm = hm.clone().squeeze().cpu().detach().numpy()  # [h, w]

        # max_ = np.max(hm)
        # hm = np.minimum(hm, max_)
        # hm /= max_
        # hm = cv2.resize(hm, (72, 72))
        # hm = np.uint8(255 * hm)
    ...
    def init_net(self, im_patches: torch.Tensor):
        ...
        target_boxes = self.init_target_boxes()[10:23].clone().cpu()
        ...
        optimizer = optim.SGD([{'params': self.net.feature_extractor.parameters()},
                               {'params': self.net.pyramid_36.parameters()},
                               {'params': self.net.pyramid_72.parameters()},
                               # {'params': self.net.regressor_72.parameters()},
                               # {'params': self.net.classifier_72.parameters()},
                               # {'params': self.net.classifier_18.parameters()},
                               {'params': self.net.pyramid_first_conv.parameters()}],
                               lr=0.001)
        ...
        for iter in range(100):
            train_id = random.randint(0, 7)
            inputs = im_patches[10:23, ...].clone()
            train_inputs = im_patches[train_id:(train_id + 1), ...].clone()
            train_boxes = self.init_target_boxes()[train_id:(train_id + 1)].clone().cuda()
            hm_18, hm_72, offset_maps = self.init_vsd_model(inputs, train_inputs, train_boxes)
            # print("hm_18 shape: {}, hm_72 shape: {}, off_map shape: {}".format(hm_18.shape, hm_72.shape, offset_maps.shape))
            # print("hm shape: {}".format(hm_72[-1].shape))
            vis_id = random.randint(0, 10)
            self.vis_hm(hm_72[-1][vis_id], iter)

            # calulate losses
            reg_loss_72 = objective['reg_72'](offset_maps, ind_72, w2h2_72, radius=1)
            clf_losses_test_72_ = objective['test_clf_72'](hm_72[0], cls_label_72)
            clf_losses_test_18_ = objective['test_clf_18'](hm_18[0], cls_label_18)
            clf_losses_test_72 = objective['test_clf_72'](hm_72[-1], cls_label_72)
            clf_losses_test_18 = objective['test_clf_18'](hm_18[-1], cls_label_18)
            tot_loss = clf_losses_test_72 * 100 + clf_losses_test_18 * 100 \
                       + clf_losses_test_72_ * 100 + clf_losses_test_18_ * 100
            # tot_loss = clf_losses_test_72 * 100 + clf_losses_test_18 * 100
            print("Train {}-iter: Total loss: {}, Clf_18 loss: {}, Clf_72 loss: {}, Reg loss: {}".format(
                iter, tot_loss.item(), clf_losses_test_18.item() * 100, clf_losses_test_72.item() * 100, reg_loss_72.item()))
    ...
    def init_classifier_and_regressor(self, init_backbone_feat):
        print("init classifier and regressor...")
        ...
        train_feat_72_reg = train_feat_72_reg[:int(self.params.reg_mem_size)]
        ...
        self.output_sz = torch.Tensor([self.feat_sz, self.feat_sz])
