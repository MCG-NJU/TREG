...
if __name__ == '__main__':
    ...
    if 'OTB' in args.dataset:
        ...
        benchmark = OPEBenchmark(dataset, tracker_name, tracker_params)
        # min_deform_rate=0.0
        # max_deform_rate=0.25
        # params = [tracker_params, min_deform_rate, max_deform_rate]
        # success_ret = {}
        # with Pool(processes=args.num) as pool:
        #     for ret, frames_deform_rate in tqdm(pool.imap_unordered(benchmark.eval_slices_success,
        #                                         tracker_params), desc='eval slices success', total=len(tracker_params),
        #                     ncols=100):
        #         success_ret.update(ret)
        #         fig = plt.figure(figsize=(12, 3.5), dpi=400)
        #         x = sns.distplot(frames_deform_rate, bins=20)
        #         x = x.get_figure()
        #         save_path = '/home/cyt/project/FCOT/deform_rates_comb.jpg'
        #         x.savefig(save_path, dpi=800)
        success_ret = {}
        with Pool(processes=args.num) as pool:
            for ret in tqdm(pool.imap_unordered(benchmark.eval_success,
                                                tracker_params), desc='eval success', total=len(tracker_params),
                            ncols=100):
                success_ret.update(ret)
        precision_ret = {}
        with Pool(processes=args.num) as pool:
            for ret, _ in tqdm(pool.imap_unordered(benchmark.eval_slices_precision,
                                                   tracker_params), desc='eval precision', total=len(tracker_params),
                            ncols=100):
                precision_ret.update(ret)
        benchmark.show_result(success_ret, precision_ret,
                              show_video_level=args.show_video_level)
