...
class LTRMaskTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, basic_device, lr_scheduler=None, logging_file="log.txt",
        ...
        """
        super().__init__(actor, loaders, optimizer, settings, basic_device, lr_scheduler, logging_file, max_norm)
    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(loader, 1):
            ...
            # forward pass
            loss, stats, vis_test_img, vis_gt_mask, vis_pred_mask = self.actor(data)
            if i % vis_inte == 0 and self.settings.local_rank in [-1, 0]:
                images = make_grid(vis_test_img, nrow=4, padding=2, normalize=True, scale_each=False, pad_value=0)
                if vis_gt_mask is not None:
                    vis_gt_mask = self.tensors_gray2rgb(vis_gt_mask.unsqueeze(0).unsqueeze(1))
                    vis_gt_mask = make_grid(vis_gt_mask, nrow=1, padding=2, normalize=False, scale_each=False, pad_value=0)
                vis_pred_mask = self.tensors_gray2rgb(vis_pred_mask.unsqueeze(0).unsqueeze(1))
                vis_pred_mask = make_grid(vis_pred_mask, nrow=1, padding=2, normalize=False, scale_each=False, pad_value=0)
                print(loader.name)
                if loader.name == 'train':
                    self.train_image_writer.add_image('test_images', images, global_step=self.train_count)
                    if vis_gt_mask is not None:
                        self.train_image_writer.add_image('vis_gt_mask', vis_gt_mask, global_step=self.train_count)
                    self.train_image_writer.add_image('vis_pred_mask', vis_pred_mask, global_step=self.train_count)
                    self.train_count += 1
                elif loader.name == 'val':
                    self.val_image_writer.add_image('test_images', images, global_step=self.val_count)
                    if vis_gt_mask is not None:
                        self.val_image_writer.add_image('vis_gt_mask', vis_gt_mask, global_step=self.val_count)
                    self.val_image_writer.add_image('vis_pred_mask', vis_pred_mask, global_step=self.val_count)
                    self.val_count += 1
                print("Displayed vis maps in tensorboard done.")
            ...
            if loader.training:
                ...
                if self.max_norm > 0:
                    torch.nn.utils.clip_grad_norm_(self.actor.net.parameters(), self.max_norm)
