...
class LTRTrainer(BaseTrainer):
    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(loader, 1):
            ...
            # forward pass
            if i % 50 == 0 and self.settings.local_rank in [-1, 0]:
                loss, stats, vis_results = self.actor(data, vis=True)
                for k,v in vis_results.items():
                    # print(k)
                    vis_img = v
                    vis_img = make_grid(vis_img, nrow=3, padding=2, normalize=True, scale_each=False, pad_value=0)
                    if loader.name == 'train':
                        self.train_image_writer.add_image(k, vis_img, global_step=self.train_count)
                    elif loader.name == 'val':
                        self.val_image_writer.add_image(k, vis_img, global_step=self.val_count)
                if loader.name == 'train':
                    self.train_count += 1
                elif loader.name == 'val':
                    self.val_count += 1
                ...
                loss, stats, vis_img = self.actor(data, vis=False)
