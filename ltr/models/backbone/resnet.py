...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dilation=1, use_bn=True, freeze_bn=False):
        super(BasicBlock, self).__init__()
        self.use_bn = use_bn
        self.conv1 = conv3x3(inplanes, planes, stride, dilation=dilation)
        if freeze_bn:
            self.norm = FrozenBatchNorm2d
        else:
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dilation=1, freeze_bn=False):
...
class ResNet(Backbone):
    ...
    def __init__(self, block, layers, output_layers, num_classes=1000, inplanes=64, dilation_factor=1, frozen_layers=(),
        ...
        self.bn1 = self.norm(inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        stride = [1 + (dilation_factor < l) for l in (8, 4, 2)]
        self.layer1 = self._make_layer(block, inplanes, layers[0], dilation=max(dilation_factor//8, 1), freeze_bn=freeze_bn)
        self.layer2 = self._make_layer(block, inplanes*2, layers[1], stride=stride[0], dilation=max(dilation_factor//4, 1), freeze_bn=freeze_bn)
        self.layer3 = self._make_layer(block, inplanes*4, layers[2], stride=stride[1], dilation=max(dilation_factor//2, 1), freeze_bn=freeze_bn)
        self.layer4 = self._make_layer(block, inplanes*8, layers[3], stride=stride[2], dilation=dilation_factor, freeze_bn=freeze_bn)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, freeze_bn=False):
        downsample = None
        if freeze_bn:
            self.norm = FrozenBatchNorm2d
        else:
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, dilation=dilation, freeze_bn=freeze_bn))
...
def resnet34(output_layers=None, pretrained=False, dilation_factor=1, **kwargs):
    """Constructs a ResNet-18 model.
    """
    ...
    model = ResNet(BasicBlock, [3, 4, 6, 3], output_layers, dilation_factor=dilation_factor, **kwargs)
