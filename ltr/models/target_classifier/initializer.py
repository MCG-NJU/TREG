...
import math
from ltr.models.layers.non_local import NONLocalBlock2D
...
class FilterInitializerNonLocal(nn.Module):
    ...
    def __init__(self, filter_size=1, feature_dim=256, feature_stride=16, pool_square=False, filter_norm=True,
        ...
        self.nonlocal_layer = NONLocalBlock2D(in_channels=feature_dim, sub_sample=True, bn_layer=False)
...
class RegFilterInitializerLinear(nn.Module):
    """Initializes a target classification filter by applying a linear conv layer and then pooling the target region.
    args:
        filter_size:  Size of the filter.
        feature_dim:  Input feature dimentionality.
    ...
    def __init__(self, filter_size=1, feature_dim=256, out_dim=5, feature_stride=4, pool_square=False, filter_norm=True,
                 conv_ksz=3, init_weights='default'):
        super().__init__()

        self.filter_conv = nn.Conv2d(feature_dim, feature_dim * out_dim, kernel_size=conv_ksz, padding=conv_ksz // 2)
        self.filter_pool = FilterPool(filter_size=filter_size, feature_stride=feature_stride, pool_square=pool_square)
        self.filter_norm = filter_norm
    ...
    def forward(self, feat, bb):
        ...
        weights = weights.reshape(weights.shape[0], self.out_dim, weights.shape[1] // self.out_dim, weights.shape[-2], weights.shape[-1])
        # if self.filter_norm:
        # norm_scale = math.sqrt(1.0 / (feat.size(-3) * feat.size(-2) * feat.size(-1)))
        # weights_centerness = weights[:, self.out_dim:, ...] * norm_scale
        # weights_reg = weights[:, :self.out_dim, ...]
        # weights = torch.cat([weights_reg, weights_centerness], dim=1)
            # weights = weights / (weights.shape[1] * weights.shape[2] * weights.shape[3])
        return weights

        # return weights.reshape(weights.shape[0], self.out_dim, weights.shape[1] // self.out_dim, weights.shape[-2], weights.shape[-1])

class RegFilterInitializerLinearV2(nn.Module):
    """Initializes a target classification filter by applying a linear conv layer and then pooling the target region.
    args:
        filter_size:  Size of the filter.
        feature_dim:  Input feature dimentionality.
...
class FilterInitializerLinearTarget(nn.Module):
    """Initializes a target classification filter by applying a linear conv layer and then pooling the target region.
    args:
        filter_size:  Size of the filter.
        feature_dim:  Input feature dimentionality.
    ...
    def forward(self, feat, bb):
        ...
        # feat = self.filter_conv(feat.view(-1, feat.shape[-3], feat.shape[-2], feat.shape[-1]))
        feat = self.filter_conv(feat)

        weights = self.filter_pool(feat, bb_pool)
