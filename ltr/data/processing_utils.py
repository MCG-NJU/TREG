...
def iou_xyxy(reference, proposals):
    """Compute the IoU between a reference box with multiple proposal boxes. box in format: x0y0x1y1

    args:
        reference - Tensor of shape (n, 4, 1).
        proposals - Tensor of shape (n, 4, num_proposals)
    ...
    # Intersection box
    tl = torch.max(reference[:, :2, :], proposals[:, :2, :])
    br = torch.min(reference[:, 2:, :], proposals[:, 2:, :])
    sz = (br - tl).clamp(0)

    # Area
    intersection = sz[:, 0, :] * sz[:, 1, :]  # nb, num_proposals
    reference_wh = reference[:, 2:, :] - reference[:, :2, :]
    proposals_wh = proposals[:, 2:, :] - proposals[:, :2, :]
    # union = reference_wh.prod(dim=1) + proposals.prod(dim=1) - intersection # nb, num_proposals
    union = reference_wh.prod(dim=1) + proposals_wh.prod(dim=1) - intersection  # nb, num_proposals
