...
import ltr.data.processing_utils_mask as prutils_SE
...
class SEMaskProcessing(SEProcessing):
    def __call__(self, data: TensorDict):
        ...
        # w, h = test_anno[:, 2] - test_anno[:, 0], test_anno[:, 3] - test_anno[:, 1]
        ...
        boxes_16 = (data['test_anno'] / 16.0).float()
        # boxes is in format xywh, convert it to x0y0x1y1 format
        boxes_16[:, 2:4] = boxes_16[:, 0:2] + boxes_16[:, 2:4]

        center_float = torch.stack(((boxes_16[:, 0] + boxes_16[:, 2]) / 2., (boxes_16[:, 1] + boxes_16[:, 3]) / 2.), dim=1)
        center_int = center_float.int().float()
        self.output_w = 16
        ind_16 = center_int[:, 1] * self.output_w + center_int[:, 0]  # [num_images, 1]

        data['ind_16'] = ind_16.long()
        data['w2h2'] = w2h2
