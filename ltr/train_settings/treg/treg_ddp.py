...
def run(settings):
    settings.description = 'Default train settings for treg, which use pretrained super-dimp, bs is 80, max_gap is 200, reg_filter_size is 7, clf_size is 4.'
    ...
    dataset_train = sampler.FCOTSampler([lasot_train, got10k_train, trackingnet_train, coco_train], [settings.lasot_rate,1,1,1],
                                        samples_per_epoch=settings.samples_per_epoch * settings.batch_size, max_gap=200, num_test_frames=3, num_train_frames=3,
    ...
    loader_train = LTRLoader('train', dataset_train, training=True, batch_size=settings.batch_size, sampler=train_sampler,
    ...
    loader_val = LTRLoader('val', dataset_val, training=False, batch_size=settings.batch_size, epoch_interval=5,
                           sampler=val_sampler, num_workers=settings.num_workers, shuffle=False, drop_last=True, stack_dim=1)

    # Create network
    net = tregnet.tregnet(clf_filter_size=settings.clf_target_filter_sz, reg_filter_size=settings.reg_target_filter_sz,
                          backbone_pretrained=True, optim_iter=5,
                          ...
                          score_act='relu', frozen_backbone_layers=['conv1', 'bn1', 'layer1', 'layer2'])
    ...
    loss_weight = {'test_clf_72': 100, 'test_init_clf_72': 100, 'test_iter_clf_72': 400,
                    ...
                    'reg_72': 1}
    actor = actors.FcotActor(net=net, objective=objective, loss_weight=loss_weight, device=settings.device)
    optimizer = optim.Adam([{'params': actor.net.module.classifier_72.filter_initializer.parameters(), 'lr': 5e-5},
                            {'params': actor.net.module.classifier_72.filter_optimizer.parameters(), 'lr': 5e-4},
                            {'params': actor.net.module.classifier_72.feature_extractor.parameters(), 'lr': 5e-5},
                            {'params': actor.net.module.classifier_18.filter_initializer.parameters(), 'lr': 5e-5},
                            {'params': actor.net.module.classifier_18.filter_optimizer.parameters(), 'lr': 5e-4},
                            {'params': actor.net.module.classifier_18.feature_extractor.parameters(), 'lr': 5e-5},
                            {'params': actor.net.module.regressor_72.parameters()},
                            {'params': actor.net.module.pyramid_first_conv.parameters()},
                            {'params': actor.net.module.pyramid_36.parameters()},
                            {'params': actor.net.module.pyramid_72.parameters()},
                            {'params': actor.net.module.feature_extractor.parameters(), 'lr': 2e-5}
                            ],
                            lr=2e-4)

    lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[28, 35, 45], gamma=0.2)

    trainer = LTRTrainer(actor, [loader_train, loader_val], optimizer, settings, settings.device, lr_scheduler,
